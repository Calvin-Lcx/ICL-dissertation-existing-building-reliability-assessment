 %%
% 6-meter Simply Supported Unrestrained Steel Beam with UDL
...
% Limite State: 
% 1.Shear resistance 2. Moment resistance 3.Deflection of thebeam 4.Lateral
...
% Using a 10% perturbation method, computed normalized sensitivity coefficients 
...
% (-0.83 Δβ/% up to -1.15 Δβ/% at t=50 years), indicating load increases
% accelerate reliability degradation
%
% Yield strength (mu_fy) showed significant positive impact (+0.46 to +0.65
% Δβ/%), demonstrating material strength's protective role.
%
% Corrosion rate (mu_kcor1) effects were negligible (<0.05 Δβ/%) with 
...
nSamples = 2e7;
current_structure_age = 30; % 30-year-old structure
design_life = 50;
...
mu_kcor1 = 0.0013; % Interior corrosion rate (mm/year) (BS EN ISO 9223(2012))
cov_kcor1 = 0.3;
...
cov_E = 0.03;
    ...
    % Cross-section geometry 457 x 191 x 98 UKB, S275 
    mu_b = 192.8;      % (mm)  Normal Distribution
    cov_b = 0.01;       
    mu_h = 467.2;      % (mm)  Normal Distribution
    cov_h = 0.01;      
    mu_tf = 19.6;      % (mm)  Normal Distribution
    cov_tf = 0.03;     
    mu_tw = 11.4;      % (mm)  Normal Distribution
    cov_tw = 0.04;    

% Loading
mu_L = 6000;      % Beam length (mm), Normal
cov_L = 0.005;
...
for i = 1:n_time
    ...
    iz = sqrt(Iz./Anet);
    % lambda_Z
    lambda_Z = (samples_L./iz)* 1./(pi * sqrt(E_t./fy_t));
    ...
    G = [g_shear, g_bending, g_deflection,g_ltb];
    ...
    betas_current = [beta_shear(i), beta_moment(i), beta_deflection(i),beta_ltb(i)];
    R = mvnrnd(zeros(1, 4), rho_failure_modes, nSamples);
...
%% Sensitivity Analysis with validation
fprintf('\nPerforming sensitivity analysis...\n');
parameters = {'mu_fy', 'mu_kcor1', 'mu_LL'};
delta = 0.10; % Increase to 10% variation for better sensitivity detection
...
for p = 1:length(parameters)
   ...
   elseif strcmp(param_name, 'mu_LL')
    ...
    X_LTmod = X_LTmod(valid_idx);
        ...
        valid_idx = ~(isinf(g_shear) | isnan(g_shear) | isinf(g_bending) | isnan(g_bending) | isinf(g_deflection) | isnan(g_deflection) | isinf(g_ltb) | isnan(g_ltb) );
        ...
        if Pf_ltb_sens < 1e-7 || isnan(Pf_ltb_sens)
            ...
            beta_ltb_sens = max(-10, min(10, -norminv(Pf_ltb_sens)));
        end

        % Correlation between failure modes
        G_sens = [g_shear(valid_idx), g_bending(valid_idx), g_deflection(valid_idx),g_ltb(valid_idx)];
        rho_failure_modes_sens = corrcoef(G_sens);
        
        % Generate correlated random variables
        betas_current_sens = [beta_shear_sens, beta_moment_sens, beta_deflection_sens, beta_ltb_sens];
        R_sens = mvnrnd(zeros(1, 4), rho_failure_modes_sens, sum(valid_idx));
...
arrays_to_check = {'beta_moment', 'Pf_moment', 'beta_deflection', 'Pf_deflection', 'beta_ltb', 'Pf_ltb'...
...
for i = 1:n_time
   ...
   % Moment limit state
    ...
    if i <= length(results) && isfield(results(i), 'rho_matrix')
        failure_modes = {'Shear', 'Moment', 'Displacement', 'LTB'};
...
fprintf('Time (years)\tShear β\t\tMoment β\tDeflection β\tLTB β\\tSystem β\n');
for i = 1:n_time
    fprintf('%d\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\n', ...
        t_points(i), beta_shear(i), beta_moment(i), beta_deflection(i), beta_ltb(i), beta_system(i));
end
...
plot(t_points, beta_ltb, 'y-s', 'LineWidth', 2, 'MarkerSize', 6);
...
legend('Shear', 'Moment', 'Deflection', 'LTB', 'System', 'Location', 'best');
grid on;
...
bar_data = [lifetime_results.shear_lifetimes, lifetime_results.moment_lifetimes, ...
           lifetime_results.deflection_lifetimes, lifetime_results.ltb_lifetimes,lifetime_results.system_lifetimes];
bar(bar_data);
...
sgtitle('Comprehensive Structural Reliability and Lifetime Analysis', 'FontSize', 16, 'FontWeight', 'bold');
 ...
 % Failure modeds correlation matrix
    figure;
    heatmap({'Shear', 'Moment', 'displacement', 'LTB'}, {'Shear', 'Moment', 'displacement', 'LTB'}, rho_failure_modes,...
...
labels = {'Shear', 'Moment', 'displacement', 'LTB'};
n = size(G, 2); 
for i = 1:n
